...
if __name__ == '__main__':
    ...
    ground_truth = []
    csvFile = open('./'+os.path.basename(os.path.dirname(gt_path))+'_val_imageset.csv','w')
    ...
    for filename in os.listdir(gt_path):
        ...
        if (os.path.isfile(filePath)):
            with open(os.path.join(gt_path,filename), 'r') as f:
                for line in f:
                    (bbox_left,bbox_top,bbox_width,bbox_height,score,object_category,truncation,occlusion ) = map(int,(line.split(',')))
                    if(object_category!=0 and object_category !=11):
                        ground_truth.append(list((os.path.splitext(filename)[0],bbox_left,bbox_top,bbox_width,bbox_height,score,object_category,truncation,occlusion)))
            ...
            for obj in sortedList:
                ...
                bbox_width = obj[3]
                bbox_height = obj[4]
                object_category = obj[6]
                #video = os.path.splitext(filename)[0]  
                row = [str(os.path.abspath(os.path.dirname(os.path.dirname(gt_path))+'/images/{}.jpg'.format(name))),str(bbox_left),str(bbox_top),str(bbox_left+bbox_width),str(bbox_top+bbox_height),str(CLASSES[int(object_category)])]
                writer.writerow(row)
